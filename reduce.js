// reduce: The reduce() method executes a user-supplied "reducer" callback function on each element of the array, in order, passing in the return value from the calculation on the preceding element. The final result of running the reducer across all elements of the array is a single value.

// .reduce(accumulatorFunction,initialvalue)
// aacumulatorFunction uses two parameters(previous/accumulator,current)

// getting the maximum element of an array

// const arrayOfNumbers = [2, 4, 24, 8, 12];
// const maximum = arrayOfNumbers.reduce((accumulator, current) => Math.min(accumulator, current), 0);
// console.log(maximum);


// reduce method: will reduce all the elements of an array into a single value
// reduce() is a callback function with two parameters: 1. accumulator and current; the accumulattor is the value that we initialize (here 0, if not given anything , it would take the 1st value which would be 2 and then current value would be 4)
// so reuce function has two argument . 1 callback function.
// 2.intitial value for the accumulator , which is 0 here

// how it works---------
// 1.   initially , a=0,c=2;Math.max(a,c)=2;so, a will be 2 after the callback function is executed
// 2.   a=2,c=4 ; a=4;
// 3.    a=4,c=24; a=24;
// 4.   a=24,c=8; a=24;
// 5. a=24,c=8; a=24;
// 6. a=24 , c=12; a=24

// example 1 : find total
const numbers = [1, 2, 3, 4, 5];
const total = numbers.reduce((previous, current) => previous + current, 0);
console.log(total);
// how it works : 
// 1.initially previous=0,current=1; previous+current=1;
// 2. previous=1, current=2; previous+current=3;
// 3.previous=3,current=3;previous+current=6;
// 4.previous=6,current=4;previous+current=10;
// 5.previous=10;current=5;previou+current=15;

// example 2: find the maximum of an array;

const max = numbers.reduce((previous, current) => Math.max(previous, current), 0);
console.log(max);
